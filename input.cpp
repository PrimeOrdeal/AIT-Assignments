...
void Input::get_input()
{
	...
	_button_state[Button::DEBUG_SHOW_COORDS] = Button_State::UP;
	_button_state[Button::PLAYER_NINJA] = Button_State::UP;
	_button_state[Button::PLAYER_ZOMBIE] = Button_State::UP;
	_button_state[Button::PLAYER_KNIGHT] = Button_State::UP;
	...
	for (auto button_state : _button_state)
	{
		...
		if (button_state.second == Button_State::RELEASED)
		{
	...
	while (SDL_PollEvent(&event))
	{
		...
		case SDL_SYSWMEVENT:
			...
			else if (event.syswm.msg->msg.win.wParam == ID_DEBUG_TOGGLECOORDINATES)
			{
				_button_state[Button::DEBUG_SHOW_COORDS] = Button_State::PRESSED;
			}
			else if (event.syswm.msg->msg.win.wParam == ID_DEBUG_TOGGLEDISPLAYID)
			{
				_button_state[Button::DEBUG_SHOW_IDS] = Button_State::PRESSED;
			}
			else if (event.syswm.msg->msg.win.wParam == ID_SKIN_NINJA)
			{
				_button_state[Button::PLAYER_NINJA] = Button_State::PRESSED;
			}
			else if (event.syswm.msg->msg.win.wParam == ID_SKIN_ZOMBIE)
			{
				_button_state[Button::PLAYER_ZOMBIE] = Button_State::PRESSED;
			}
			else if (event.syswm.msg->msg.win.wParam == ID_SKIN_KNIGHT)
			{
				_button_state[Button::PLAYER_KNIGHT] = Button_State::PRESSED;
			}
			break;
		case SDL_QUIT:
			_button_state[Button::QUIT] = Button_State::PRESSED;
