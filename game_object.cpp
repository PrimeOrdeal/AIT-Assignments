...
Game_Object::Game_Object(std::string id, std::string texture_id) : _translation(0,0), _velocity(0,0), _collider(0.0f, Vector_2D(0.f, 0.f))
{
	_id = id;
	_texture_id = texture_id;

	_width = 100; // default width
	_height = 100; // defaul height
...
void Game_Object::simulate_physics(Uint32 milliseconds_to_simulate, Assets*, Scene* scene)
{
	...
	// Each Game Object should detect the collision with every other Game Object and then repulse away if a collision happened.
	for (Game_Object* game_object : scene->get_game_objects())
	{
		// Don't test if asset is colliding with itself.
...
void Game_Object::render(Uint32, Assets* assets, SDL_Renderer* renderer, Configuration* config)
{
	...
	// Flip the texture if the direction is less than PI / 2 (less than 90*).
	...
	else if (config->should_display_coordinates)
	{
		SDL_Color text_color;
		text_color.r = 255;
		text_color.g = 255;
		text_color.b = 0;
		text_color.a = 255;
		std::string coord_x = std::to_string(destination.x);
		std::string coord_y = std::to_string(destination.y);

		Text id(renderer, (coord_x + " " + coord_y).c_str(), text_color, "Coord.Text");
		id.render(renderer, _translation + Vector_2D((float)_width / 2, (float)_height));
	}
	else if (config->should_display_colliders)
	{
		Texture* collider_texture = (Texture*)assets->get_asset("Texture.Collider");

		// This code will translate the collider to the game object position + the collider position, 
		// and then subtract the radius (half the width of the collider).
		...
		// The width and height are double the radius (effectively the diameter).
...
// This allows for the player skin to be changed through the menu options.
void Game_Object::set_texture(std::string texture_id)
{
	_texture_id = texture_id;
}
